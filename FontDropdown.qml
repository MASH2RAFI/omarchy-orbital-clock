...
// Scrollable dropdown for medium-length option lists (font pickers).
Item {
  id: root
  ...
  readonly property int popupListHeight: Math.min(
    root.options.length * root.popupRowHeight + Math.max(0, root.options.length - 1) * Style.spacing.labelGap,
    root.popupRowHeight * root.maxVisibleRows + Math.max(0, root.maxVisibleRows - 1) * Style.spacing.labelGap
  )
  ...
  Column {
    ...
    BorderSurface {
      ...
      Popup {
        ...
        width: trigger.width
        implicitHeight: listContainer.height + popup.topPadding + popup.bottomPadding
        ...
        contentItem: Item {
          id: listContainer
          width: trigger.width - popup.leftPadding - popup.rightPadding
          height: root.popupListHeight
